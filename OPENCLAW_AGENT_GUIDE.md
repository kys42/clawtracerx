...
[System Prompt]
  ├── 플랫폼 기본 지시 (hardcoded)
  ├── "# Project Context"          ← PROJECT 영역 시작
  │   ├── AGENTS.md 내용
  │   ├── SOUL.md 내용
  │   ├── TOOLS.md 내용
  │   ├── ... (나머지 부트스트랩 파일)
  │   └── (bootstrapMaxChars 제한으로 잘릴 수 있음)
  ├── 스킬 블록 (skills.entries[]) ← SYSTEM 영역 (Skills/Tools는 SYSTEM에 포함)
...
### 17.4 PROJECT vs SYSTEM 구분 (systemPromptReport 기준)

`systemPromptReport`의 토큰/크기 분류:

| 구분 | 내용 | `systemPromptReport` 필드 |
|------|------|--------------------------|
| **PROJECT** | 워크스페이스 파일 (`# Project Context` 블록) | `systemPrompt.projectContextChars` |
...
4. **sessionKey vs sessionId**: 서브에이전트에서 이 둘은 완전히 다른 UUID — `childSessionKey`의 UUID는 게이트웨이 라우팅용이므로 파일명이 아님. **실제 JSONL 파일명 UUID는 announce 메시지의 sessionId에서만 얻을 수 있음**
...
8. **announce chunk 파싱 주의**: regex match end 기준으로 텍스트를 자르면 Stats 인라인 필드(`• sessionId`, `• transcript`)를 놓침 → 빈 줄(`\n\n`) 기준으로 chunk 끝을 잡아야 함
9. **중복 레이블 서브에이전트**: 같은 레이블로 여러 번 spawn하면 announce와 spawn을 순서대로 매칭해야 함 (`pop(0)` 방식)
...
JSONL 사후 분석 외에 **실시간 에이전트 인터랙션** 기능:
- 메시지 전송 → 도구 호출/서브에이전트 실행 실시간 관찰
- `deliver: false` — 텔레그램/디스코드로 배달 없이 테스트
- `extraSystemPrompt` — 추가 지시 주입 (hooks 없이도 가능)
- 워크스페이스 파일 직접 편집 (`.lab-backup` 자동 생성)

### 20.2 게이트웨이 WS RPC `agent` 메서드

Lab이 에이전트에 메시지를 보내는 방식:
...
```json
{
  ...
  "params": {
    "message": "메시지 본문",
    "sessionKey": "agent:main:chat:lab-test",
    "agentId": "main",
    "deliver": false,
    "model": null,
    "thinking": null,
    "extraSystemPrompt": "추가 지시",
    "timeout": 120,
    "idempotencyKey": "uuid"
  }
}
...
응답은 `payload` 필드에 있음 (`result` 아님).

### 20.3 폴링 전략

에이전트 실행 완료 감지: `parser.py`의 마지막 턴 `stop_reason == "stop"` 확인.

- **fast poll** (2초): 메시지 전송 직후 — 응답 빠르게 확인
- **slow poll** (8초): 세션 감시 중 — 항상 켜두되 부하 줄임
- `done` 신호 수신 후 → fast → slow 전환 (폴링 완전 종료 없음)

### 20.4 Lab 세션 키 패턴

```
agent:{agentId}:chat:{임의식별자}
예) agent:main:chat:lab-test-001
```
